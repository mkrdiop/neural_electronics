...
# Function to simulate the behavior of an 8-to-1 Multiplexer (MUX)
def simulate_8_to_1_mux(inputs, control_inputs):
...
# Function to generate a dataset for the 8-to-1 MUX
def generate_8_to_1_mux_dataset(num_samples):
    inputs = np.random.randint(0, 2, size=(num_samples, 8))  # Random binary data inputs (D0 to D7)
    control_inputs = np.random.randint(0, 2, size=(num_samples, 3))  # Random binary control inputs (S0, S1, S2)
    outputs = simulate_8_to_1_mux(inputs, control_inputs)  # Simulate the MUX output

    return inputs, control_inputs, outputs

# Function to create a neural network model for 8-to-1 MUX emulation
def create_8_to_1_mux_model():
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(16, activation='relu', input_shape=(11,)),  # Hidden layer with 16 neurons
...
# Generate a dataset for the 8-to-1 MUX
num_samples = 10000  # Increase the dataset size
inputs, control_inputs, outputs = generate_8_to_1_mux_dataset(num_samples)
...
mux_model = create_8_to_1_mux_model()
